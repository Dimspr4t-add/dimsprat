...
const STORE_OTS_TICKETS = 'otsTickets';
...
function initDB() {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // Create OTS tickets store if it doesn't exist
      if (!db.objectStoreNames.contains(STORE_OTS_TICKETS)) {
        const otsStore = db.createObjectStore(STORE_OTS_TICKETS, {
          keyPath: 'id',
          autoIncrement: true
        });
        otsStore.createIndex('status', 'status', { unique: false });
        otsStore.createIndex('timestamp', 'timestamp', { unique: false });
        otsStore.createIndex('syncStatus', 'syncStatus', { unique: false });
      }
...
async function addToSyncQueue(action, data) {
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      const result = { ...syncItem, id: request.result };
      // Trigger sync if online
      if (navigator.onLine) {
        self.registration.sync.register('sync-queue').catch(console.error);
      }
      resolve(result);
    };
...
// OTS Tickets operations
async function saveOtsTicket(otsData) {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([STORE_OTS_TICKETS], 'readwrite');
    const store = transaction.objectStore(STORE_OTS_TICKETS);
    
    // Add timestamp if not exists
    if (!otsData.timestamp) {
      otsData.timestamp = new Date().toISOString();
    }
    ...
    const request = store.add(otsData);
    
    request.onsuccess = () => {
      // Add to sync queue if it's a new record
      if (!otsData.id) {
        otsData.id = request.result;
        addToSyncQueue('addOtsTicket', { ...otsData, id: request.result })
          .then(() => resolve({ ...otsData, id: request.result }))
          .catch(error => {
            console.error('Error adding to sync queue:', error);
            resolve({ ...otsData, id: request.result }); // Still resolve if queue fails
          });
      } else {
        resolve(otsData);
      }
    ...
    request.onerror = (event) => {
      console.error('Error saving OTS ticket:', event.target.error);
      reject('Error saving OTS ticket: ' + event.target.error);
    };
  });
}

async function getAllOtsTickets() {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([STORE_OTS_TICKETS], 'readonly');
    const store = transaction.objectStore(STORE_OTS_TICKETS);
    const request = store.getAll();
    
    request.onsuccess = () => resolve(request.result || []);
    request.onerror = (event) => {
      console.error('Error getting OTS tickets:', event.target.error);
      reject('Error getting OTS tickets: ' + event.target.error);
    };
  });
}

async function updateOtsStatus(id, status) {
  return new Promise((resolve, reject) => {
    const transaction = db.transaction([STORE_OTS_TICKETS], 'readwrite');
    const store = transaction.objectStore(STORE_OTS_TICKETS);
    ...
    getRequest.onsuccess = () => {
      ...
      const updateRequest = store.put(updatedTicket);
      
      updateRequest.onsuccess = () => resolve(updatedTicket);
      updateRequest.onerror = (event) => {
        console.error('Error updating OTS ticket:', event.target.error);
        reject('Error updating OTS ticket: ' + event.target.error);
      };
    };
    
    getRequest.onerror = (event) => {
      console.error('Error getting OTS ticket:', event.target.error);
      reject('Error getting OTS ticket: ' + event.target.error);
    };
  });
}
