...
// Configuration
const CONFIG = {
  SPREADSHEET_ID: '1CGa7NAkAIBwxoGmaFmo0Tsm3IZoXFeDBtQypq_GTcbw', // Replace with your Google Sheet ID
  ...
  try {
    ...
    switch (action) {
      ...
      case 'checkInTicket':
        ...
        break;
      case 'syncTickets':  // Tambahkan ini
        response = handleSyncTickets(e);
        break;
      case 'getLatestData':  // Tambahkan ini
        response = getLatestData(e);
        break;
      default:
        response = createResponse(400, { error: 'Aksi tidak valid' });
    }
...
function handleSyncTickets(e) {
  try {
    const syncData = JSON.parse(e.parameter.data || '[]');
    const result = {
      success: [],
      failed: []
    };
    ...
    syncData.forEach(item => {
      try {
        if (item.action === 'checkIn') {
          const response = checkInTicket(item.ticketId);
          result.success.push({
    ...
    return createResponse(500, { 
      success: false, 
      error: 'Gagal memproses sinkronisasi',
      details: error.message 
    });
  }
}
...
function getLatestData(e) {
  try {
    const lastSyncTime = e.parameter.lastSyncTime || 0;
    const sheet = getSheet();
    const data = sheet.getDataRange().getValues();
    const headers = data[0].map(h => h.toString().toLowerCase().trim());
    ...
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      const ticketTime = new Date(row[timestampCol] || 0);
      
      if (ticketTime > new Date(lastSyncTime)) {
        const ticket = {};
